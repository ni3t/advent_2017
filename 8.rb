...
Instructions = []
Instruction = Struct.new(:name, :op, :mod, :other, :otherop, :otherval)
Register = Struct.new(:name, :value)
Registers = []
...
input = (ARGV.delete("-t") ? TEST : DATA).each_line.map do |line|
  name = line.split[0]
  op = line.split[1] == "inc" ? :+ : :-
  mod = line.split[2].to_i
  other = line.split[4]
  otherop = line.split[5].to_sym
  otherval = line.split[6].to_i
  Instructions << Instruction.new(name, op, mod, other, otherop, otherval)
  if Registers.find { |r| r.name == name }.nil?
    Registers << Register.new(name, 0)
  end
end

Instructions.map do |i|
  if Registers.find { |r| r.name == i.other }.value.send(i.otherop, i.otherval)
    r = Registers.find { |r| r.name == i.name }
    r.instance_eval { |r| r.value = r.value.send(i.op, i.mod) }
    max = [max, r.value].max
  end
end

puts Registers.max_by(&:value).value
puts max
