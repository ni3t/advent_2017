...
# sub-sub-sub-...-towers, many programs stand simply keeping the disc below them balanced but
...
Tower = OpenStruct.new(programs: [])
Program = Struct.new(:name, :weight, :children) {
  def associate_children
    if children
      self.children = children&.map do |child|
        Tower.programs.filter { |program| program.name == child }.first
      end
    end
  end

  def child_count
    self.children.count + self.children.map(&:child_count).sum
  end

  def child_weights
    self.children&.map(&:weight)
  end

  def weight_calc
    self.weight + self.child_weights.sum
  end

  def not_balanced
    child_weights.uniq.count > 1
  end

  def unbalanced_child
    if children.group_by(&:weight_calc).length > 1
      should_be = children.group_by(&:weight_calc).filter { |k, v| v.count > 1 }&.first&.first
    end
  end
}

input = (ARGV.delete("-t") ? TEST : DATA).each_line.map do |line|
  left, right = line.chomp.split("->").map(&:chomp)

  leftmatch = left.match(/(\w+) \((\d+)\)/)
  name = leftmatch[1]
  weight = leftmatch[2].to_i

  children = right&.split(",")&.map(&:strip) || []

  Tower.programs << Program.new(name, weight, children)
end

Tower.programs.map(&:associate_children)

puts Tower.programs.max_by(&:child_count).name

puts Tower.programs.map(&:unbalanced_child).compact.first
