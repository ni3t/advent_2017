...
input = (ARGV.delete("-t") ? TEST2 : DATA).each_line.map do |line|
  line.chomp.split(" ")
end

puts input.map { |phrase| phrase.uniq.count == phrase.count }.count(true)
part2 = input.map do |phrase|
  res = phrase.map do |word|
    word.split("").sort
  end.uniq.count == phrase.size ? 1 : 0
end
